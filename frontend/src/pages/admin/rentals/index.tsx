...
import { getRentalDetails, RentalWithDetails } from '../../../data/rentals.ts';

export const AdminRentals: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<string>('all');
  const [currentPage, setCurrentPage] = useState(1);
  const [allRentals, setAllRentals] = useState<RentalWithDetails[]>([]);
  const itemsPerPage = 10;

  useEffect(() => {
    getRentalDetails().then(setAllRentals);
  }, []);
  ...
  const filteredRentals = useMemo(() => {
    return allRentals.filter(rental => {
      const searchMatch =
          ...
          rental.car?.modelName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
      <div>
        <div className="mb-6">
          ...
          <p className="text-gray-600">View and manage all car rentals.</p>
        </div>
                    ...
                    <TableRow key={rental.id}>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">{rental.car?.brand} {rental.car?.modelName}</p>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
